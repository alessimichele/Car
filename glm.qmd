...
scores
```
Since deviances are very similar and AIC is slightly lower for the reduced model, we shall choose the latter.
Moreover, a LRT shows that there is no evidence toward rejecting the simpler model
```{r}
...
scores
```
From a comparison with the first two models we see that both the deviance and AIC increase significantly if we remove Make, thus we are justified in keeping it.
Further evidence comes from a LRT comparing the reduced and the no make model.
```{r}
